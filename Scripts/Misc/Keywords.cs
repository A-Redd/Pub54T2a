...
namespace Server.Misc
{
        ...
        public static void EventSink_Speech(SpeechEventArgs args)
        {
            ...
            if (args.Speech == "lum lum lum")
            {
                if (from is PlayerMobile && ((PlayerMobile)from).HumilityHunt && (DateTime.UtcNow > ((PlayerMobile)from).HumilityHuntLastEnded + TimeSpan.FromSeconds(30)))
                {
                    ...
                    mTimer.Start();
                    from.SendAsciiMessage("Your journey on the Path of the Humble will end in 30 seconds, at that time your resists will be returned to normal"); //Your journey on the Path of the Humble will end in ~1_SECONDS~ seconds, at that time your resists will be returned to normal. 
                }
                         ...
                         (DateTime.UtcNow > ((PlayerMobile)from).HumilityHuntLastEnded + TimeSpan.FromSeconds(60)))
                {
                    ...
                    from.SendAsciiMessage("You have begun your journey on the Path of Humility.  Your resists have been debuffed by -70"); //You have begun your journey on the Path of Humility.  Your resists have been debuffed by ~1_DEBUFF~.
                    from.SendGump(new HumilityGump());

                }
                else
                {
                    from.SendAsciiMessage("You must wait before you can once again begin your journey on the Path of Humility.");
    ...
    public class HumilityTimer : Timer
    {
        ...
        protected override void OnTick()
        {
            ...
            pm.SendAsciiMessage("You have ended your journey on the Path of Humility."); // You have ended your journey on the Path of Humility.
            Stop();
        }
    }
}